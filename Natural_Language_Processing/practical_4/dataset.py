...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        truncate_len = min(document_len, truncate_len)
        ...
        masked_content_len = random.randint(int(1/8 * truncate_len), int(3/8 * truncate_len))
        masked_content_len = int(np.clip(masked_content_len, 1, truncate_len - 2))

        masked_content_idx = random.randint(1, truncate_len - masked_content_len - 1)
        prefix = truncated_doc[:masked_content_idx]
        masked_content = truncated_doc[masked_content_idx: masked_content_idx + masked_content_len]
        suffix = truncated_doc[masked_content_idx + masked_content_len:]
